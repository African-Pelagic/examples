...
class SQLCompiler(ExpressionCompiler):
    ...
    def visit_Dataset(self, expr: Dataset) -> str:
        return expr.name

    def visit_Feature(self, expr: Feature) -> str:
        if expr.dataset:
    ...
    def visit_Average(self, expr: Average) -> AggregationPlan:
        feature_sql = self.visit(expr.child)
        alias = expr.alias or f"avg_{expr.child.name}"
        return AggregationPlan("AVG", feature_sql, alias)

    def visit_GroupBy(self, expr: GroupBy) -> str:
        source_sql = self.visit(expr.source)
        plan = self.visit(expr.metric)
    ...
    def visit_Filter(self, expr: Filter) -> str:
        source_sql = self.visit(expr.source)
        value_sql = self.visit(expr.value)
...
class PandasCompiler(ExpressionCompiler):
    ...
    def visit_Dataset(self, expr: Dataset) -> pd.DataFrame:
    ...
    def visit_Average(self, expr: Average) -> AggregationPlan:
        feature_name = self.visit(expr.child)
        alias = expr.alias or f"avg_{feature_name}"
        return AggregationPlan("AVG", feature_name, alias)

    def visit_GroupBy(self, expr: GroupBy) -> pd.DataFrame:
        df = self.visit(expr.source)
        plan = self.visit(expr.metric)
    ...
    def visit_Filter(self, expr: Filter) -> pd.DataFrame:
        df = self.visit(expr.source)
